...
* **Definición matemática (Ej. Regresión Lineal):** Un modelo matemático que intenta encontrar la relación óptima entre variables. Por ejemplo, predecir ventas (Target, $y$) basándose en gastos de publicidad (Feature, $x$). El modelo $y = wx + b$ aprende los **parámetros** $w$ (peso) y $b$ iterando desde valores arbitrarios ($f\_1$) hasta un valor óptimo ($f\_3$) que minimiza el error.
...
* `df.dropna(subset=['col_name'])`: Elimina filas que tienen `NaN` específicamente en la columna 'col\_name'.
...
* **Regularización:** Técnica para prevenir el overfitting en modelos lineales penalizando coeficientes (pesos) grandes.
    * **Ridge (L2):** Añade una penalización $\\lambda\\sum{w\_j^2}$. Encoge los pesos, pero no los hace cero.
    * **Lasso (L1):** Añade una penalización $\\lambda\\sum{|w\_j|}$. Puede forzar que algunos pesos sean exactamente cero, realizando una selección de características automática.
...
* **Métricas Clave:**
    ...
    * **F1-Score:** La media armónica de Precision y Recall. Es una métrica excelente para datasets desbalanceados. $F\_1 = 2 \\frac{Precision \\times Recall}{Precision + Recall}$.
